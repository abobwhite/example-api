...
public class SupplierServiceTest {
    ...
    public void getSupplier() throws Exception {
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.get("/suppliers/" + publicSupplierId).accept(MediaType.APPLICATION_JSON)
                        .principal(authentication)).andDo(MockMvcResultHandlers.print())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.jsonPath("$.supplier.id").value("Y40dgNWM"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.supplier.gender").value("Male"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.supplier.partyType").value("Supplier"));
    }
}
