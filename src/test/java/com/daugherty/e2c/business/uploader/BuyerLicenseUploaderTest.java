...
public class BuyerLicenseUploaderTest {
    @Mock
    ...
    @InjectMocks
    private final BuyerLicenseUploader buyerLicenseUploader = new BuyerLicenseUploader();

    @Test
    public void uploadDocumentReturnsURL() throws IOException {
        Long id = 1L;
        String documentKey = DocumentType.BUYER_LICENSE.getType() + id;
        ...
        buyer.setImportLicenseRefId("oldImportLicenseRefId");
        ...
        String uploadedDocumentKey = buyerLicenseUploader.uploadDocument(id, contentType, inputStream);

        assertThat(uploadedDocumentKey, is(documentKey));
        assertThat(buyer.getImportLicenseRefId(), is(documentKey));
