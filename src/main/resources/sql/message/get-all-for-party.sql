SELECT DISTINCT
  ...
  m.subject,
  m.message_type,
  CASE WHEN m.message_id IS NOT NULL THEN FALSE ELSE FALSE END AS flagged,
  li.last_interaction_sent_time,
  FALSE as all_interactions_read
FROM message m
  ...
  INNER JOIN (SELECT message_id, max(sent_timestamp) AS last_interaction_sent_time, min(read_by_receiver) AS all_interactions_read
		          FROM interaction
		          GROUP BY message_id) AS li ON m.message_id = li.message_id
