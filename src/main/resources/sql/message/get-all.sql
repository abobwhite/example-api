SELECT
  ...
  li.last_interaction_sent_time,
  li.all_interactions_read
FROM message m
  ...
  LEFT OUTER JOIN (SELECT message_id
	...
	INNER JOIN (SELECT message_id, from_party_id, to_party_id, max(sent_timestamp) AS last_interaction_sent_time, min(read_by_receiver) AS all_interactions_read
		          FROM interaction
		          GROUP BY message_id, from_party_id, to_party_id) AS li ON m.message_id = li.message_id
